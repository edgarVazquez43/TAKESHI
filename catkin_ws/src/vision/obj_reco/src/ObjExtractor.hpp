...
static std::vector<DetectedObject> GetObjectsInFloor(cv::Mat pointCloud);
static std::vector<DetectedObject> GetObjectsInPlane(cv::Mat pointCloud,Plane3D plane);
...
static std::vector<PlanarSegment>  GetHorizontalPlanes(cv::Mat pointCloud, double minHeight, double maxHeight);
