...
std::vector<PlanarSegment>  ObjExtractor::GetHorizontalPlanes(cv::Mat pointCloud, double minHeight, double maxHeight)
{
								// PARAMS: Valid Points
								//Find planes on a slice of world between min and max height
								...
								double minObjDistToPlane = 0.005;
								...
								cv::inRange(pointCloud, cv::Scalar(-3.0, -3.0, minHeight), cv::Scalar(3.0, 3.0, maxHeight), validPointCloud);
...
//coyo was here
//Por que hardcodeo todo sR JEbus por QUEUEUEU!!!"!"

std::vector<DetectedObject> ObjExtractor::GetObjectsInPlane(cv::Mat pointCloud, Plane3D plane)
{
								double minObjDistToPlane = 0.01;
								double maxObjDistToPlane = 0.25;
								double maxDistBetweenObjs = 0.05;
								double floorDistRemoval = -0.1;
								std::vector< DetectedObject > detectedObjectsList;
								std::vector< cv::Point3f > objectsPoints;
								std::vector< cv::Point2i > objectsIdx;

								// For removing floor and far far away points
								cv::Mat mask,validPointCloud;
								cv::inRange(pointCloud, cv::Scalar(-2.0, -2.0, floorDistRemoval), cv::Scalar(2.0, 2.0, 2.0), mask);
								pointCloud.copyTo(validPointCloud, mask);

								for(int row=0; row<validPointCloud.rows; row++)
								{
																for(int col=0; col<validPointCloud.cols; col++)
																{
																								cv::Point3f xyzPoint = validPointCloud.at< cv::Vec3f >(row, col );
																								double dist = plane.DistanceToPoint(xyzPoint, true);
...
std::vector<DetectedObject> ObjExtractor::GetObjectsInFloor(cv::Mat pointCloud)
{
								std::vector< DetectedObject > detectedObjectsList;
								double ticks;
								...
								// For removing floor and far far away points
								cv::Mat validPointCloud;
								cv::inRange(pointCloud, cv::Scalar(-1.0, -1.0, floorDistRemoval), cv::Scalar(1.0, 1.0, 0.60), validPointCloud);
...
std::vector<DetectedObject> ObjExtractor::GetObjectsInHorizontalPlanes(cv::Mat pointCloud)
{
								...
								int minPointsForPlane = pointCloud.rows*pointCloud.cols*0.025;
								// PARAMS: Object Extracction
								double minObjDistToPlane = 0.005; //was maxDistToPlane
								double maxObjDistToPlane = 0.25;
								double minDistToContour = 0.02;
								double maxDistBetweenObjs = 0.01;

								// For removing floor and far far away points
								cv::Mat validPointCloud;
								cv::inRange(pointCloud, cv::Scalar(-1, -1.5, floorDistRemoval), cv::Scalar(2.0, 1.5, 2.0), validPointCloud);
								...
								for( int i=0; i<(int)horizontalPlanes.size(); i++)
								{
																cv::Point3f n = horizontalPlanes[i].Get_Plane().GetNormal();
																std::cout << "Plane Normal" << horizontalPlanes[i].Get_Plane().GetNormal()<< '\n';
																for(int row=0; row<pointCloud.rows; row++)
																{
																								for(int col=0; col<pointCloud.cols; col++)
																								{
								...
								for(int i=0; i<(int)objIdxClusters.size(); i++)
								{
																...
																//Estos son los aprametros aceptables
																//Cualquier objecto mayor a los numeritos del shadow o heigh
																//en centimetros o menor es ignorado
																objHeight = std::abs(maxZ - minZ);
																objCentroid *= (1.0f / (float)objIdxClusters[i].size());

																DetectedObject detObj = DetectedObject( objIndexes, objPoints3D, objPoints2D, objHeight, objCentroid, oriMask );
																if( detObj.shadowOriBoundBoxt2D.size.width <0.01 || detObj.shadowOriBoundBoxt2D.size.width > 0.30 )
																								continue;
																if( detObj.shadowOriBoundBoxt2D.size.height<0.01 || detObj.shadowOriBoundBoxt2D.size.height > 0.30 )
																								continue;
																if( detObj.height <0.01 || detObj.height > 0.25)
																								continue;
...
std::vector<PlanarSegment> ObjExtractor::ExtractHorizontalPlanesRANSAC(cv::Mat pointCloud, double maxDistPointToPlane, int maxIterations, int minPointsForPlane, cv::Mat mask)
{
								...
								while( cntIteratiosn++ < maxIterations && indexes.size() > minPointsForPlane)
								{
																...
																//Check if normal is non negative

																if (pcaNormal2.z<0.0) {
																								pcaNormal2*=-1;
																								// std::cout << "Found negative normal correcting" << '\n';
																								// std::cout << "Ok normal found: " << pcaNormal2<< '\n';
																}
