...
#define OP_SKEL_MIDDLE_HIP 8 //#define OP_SKEL_RIGHT_HIP 8
...
#define OP_SKEL_RIGHT_EYE 15
#define OP_SKEL_LEFT_EYE 16
#define OP_SKEL_RIGHT_EAR 17
#define OP_SKEL_LEFT_EAR 18
#define OP_SKEL_LEFT_BIG_TOE 19
#define OP_SKEL_LEFT_SMALL_TOE 20
#define OP_SKEL_LEFT_HEEL 21
#define OP_SKEL_RIGHT_BIG_TOE 22
#define OP_SKEL_RIGHT_SMALL_TOE 23
#define OP_SKEL_RIGHT_HEEL 24
#define OP_SKEL_BACKGROUND 25
...
ros::Subscriber * subPointCloud_ptr=NULL;
...
void printMessage(std::string message){
    std::cout << "\033[1;34m     OpenPose node.->" << message << "\033[0m" << std::endl;
}

void printErrorMessage(std::string message){
    std::cout << "\033[1;31m     OpenPose node Error!.->" << message << "\033[0m" << std::endl;
}
...
bool initLinksRestrictions(std::string fileXML){
        ...
        if(!isLoad) {
                printErrorMessage("Can't not open the xml file config of the openpose node.");
                return false;
        }
...
void pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& msg){
        ...
        ros::Time currTimeFrame = msg->header.stamp;
        ...
        //TODO: pass frame as rosparameter !!!
        //Right now is harcodeado
        vision_msgs::Skeletons skeletons;
        skeletons.header.frame_id = msg->header.frame_id;
        ...
        for(int i = 0; i < keyPoints.size(); i++) {
                ...
                //Debug line
                //std::cout << "lifetime " << marker.lifetime<<  '\n';
                        ...
                        if(score >= FLAGS_min_score_pose && blackList.find(OP_SKEL_NECK) == blackList.end()) {
                            ...
                            switch(it->first) {
                                ...
                                case OP_SKEL_LEFT_WRIST:
                                        ...
                                        skeleton2D.joints.push_back(joint);
                                        break;
                                case OP_SKEL_MIDDLE_HIP:
                                        joint.name_joint.data = "middle_hip";
                                        skeleton2D.joints.push_back(joint);
                                        break;
                                ...
                                case OP_SKEL_LEFT_ANKLE:
                                        ...
                                        skeleton2D.joints.push_back(joint);
                                        break;
                                case OP_SKEL_RIGHT_EYE:
                                        joint.name_joint.data = "right_eye";
                                        skeleton2D.joints.push_back(joint);
                                        break;
                                case OP_SKEL_LEFT_EYE:
                                        joint.name_joint.data = "left_eye";
                                        skeleton2D.joints.push_back(joint);
                                        break;
                                case OP_SKEL_RIGHT_EAR:
                                        joint.name_joint.data = "right_ear";
                                        skeleton2D.joints.push_back(joint);
                                        break;
                                case OP_SKEL_LEFT_EAR:
                                        joint.name_joint.data = "left_ear";
                                        skeleton2D.joints.push_back(joint);
                                        break;
                                case OP_SKEL_LEFT_BIG_TOE:
                                        joint.name_joint.data = "left_big_toe";
                                        skeleton2D.joints.push_back(joint);
                                        break;
                                case OP_SKEL_LEFT_SMALL_TOE:
                                        joint.name_joint.data = "left_small_toe";
                                        skeleton2D.joints.push_back(joint);
                                        break;
                                case OP_SKEL_LEFT_HEEL:
                                        joint.name_joint.data = "left_heel";
                                        skeleton2D.joints.push_back(joint);
                                        break;
                                case OP_SKEL_RIGHT_BIG_TOE:
                                        joint.name_joint.data = "right_big_toe";
                                        skeleton2D.joints.push_back(joint);
                                        break;
                                case OP_SKEL_RIGHT_SMALL_TOE:
                                        joint.name_joint.data = "right_small_toe";
                                        skeleton2D.joints.push_back(joint);
                                        break;
                                case OP_SKEL_RIGHT_HEEL:
                                        joint.name_joint.data = "right_heel";
                                        skeleton2D.joints.push_back(joint);
                                        break;
                                case OP_SKEL_BACKGROUND:
                                        joint.name_joint.data = "background";
                                        skeleton2D.joints.push_back(joint);
                                        break;
                                default:
                                        printErrorMessage("No valid joint id:" + std::to_string(it->first));
                                        break;
                                }
                        }
                ...
                //        int y1 = round(k1[1]);
                for(std::map<int, std::vector<float> >::iterator it = keyPoints[i].begin(); it != keyPoints[i].end(); ++it) {
                        int x1 = round(it->second[0]);
                        int y1 = round(it->second[1]);
                        vision_msgs::SkeletonJoint joint;
                        ...
                        case OP_SKEL_BACKGROUND:
                                ...
                                break;
                        default:
                                printErrorMessage("No valid joint id.");
                                break;
                        }
...
void enableEstimatePoseCallback(const std_msgs::Bool::ConstPtr& enable){
        if(enable->data) {
                printMessage("Enable Pose estimator");
                ...
                printMessage("Disable Pose estimator");
                if(subPointCloud_ptr != NULL) {
                        subPointCloud_ptr->shutdown();
                        subPointCloud_ptr = NULL;
                }
...
int main(int argc, char ** argv){
        ...
        printMessage("Initializing the openpose node by Rey Martell modified by Edd");
        ros::NodeHandle nh;
        nh_ptr = &nh;
        ros::Rate rate(30);
        ...
        if(FLAGS_debug_mode){
            printMessage("The node will be initializing with the next parameters");
            printMessage("Debug mode:" + FLAGS_debug_mode);
            printMessage("Model folder:" + FLAGS_model_folder);
            printMessage("Model pose:" + FLAGS_model_pose);
            printMessage("Net resolution:" + FLAGS_net_resolution);
            printMessage("Resolution:" + FLAGS_resolution);
            printMessage("Num gpu start:" + FLAGS_num_gpu_start);
            printMessage("Scale gap:" + std::to_string(FLAGS_scale_gap));
            printMessage("Scale number:" + FLAGS_scale_number);
            printMessage("Render threshold:" + std::to_string(FLAGS_render_threshold));
            printMessage("rgbd camera topic:" + FLAGS_rgbd_camera_topic);
            printMessage("Result pose topic:" + FLAGS_result_pose_topic);
