...
void callbackCmdNavigation(
	...
	if (tokens[1] == "person") {
		...
		if (nfp)
			success = JustinaTasks::sayAndSyncNavigateToLoc(tokens[1], 70000);
	}
	if (success)
		responseMsg.successful = 1;
	else
...
void callbackCmdAnswer(const knowledge_msgs::PlanningCmdClips::ConstPtr& msg) {
	...
	if (success) {
		...
		if (param1.compare("a_question") == 0) {
			...
			JustinaHRI::waitAfterSay("Hellow my name is takeshi", 2000);
			responseMsg.successful = 1;
			...
			JustinaHRI::waitAfterSay("My name is Takeshi", 2000);
			...
			JustinaHRI::waitAfterSay("Hello my name is Takeshi", 4000);
...
void callbackCmdFindObject(
	...
	int timeout = (fplan == true) ? (maxTime - (int)d.toSec() )*500 : maxTime * 500;
	...
	if (success && nfp) {
		...
		ss.str("");
		if (tokens[0] == "person") {
			success = JustinaTasks::findPerson("", -1, JustinaTasks::NONE, false);
			...
			success = JustinaTasks::findPerson(tokens[1], -1, JustinaTasks::NONE, false);//success = JustinaTasks::findPerson(tokens[1])
			ss << "find_spc_person " << tokens[0] << " " << tokens[1] << " " << tokens[2];//ss << responseMsg.params;
        }else if (tokens[0] == "specific_eegpsr"){
            success = JustinaTasks::findPerson(tokens[1], -1, JustinaTasks::NONE, true);//success = JustinaTasks::findPerson(tokens[1])
			ss << "find_spc_person " << tokens[0] << " " << tokens[1] << " " << tokens[2];//ss << responseMsg.params;
		...
		case SM_NAVIGATE_TO_THE_LOCATION:
			...
			if (!JustinaTasks::sayAndSyncNavigateToLoc("arena", 120000)) {
				...
				if (!JustinaTasks::sayAndSyncNavigateToLoc("arena", 120000)) {
					...
					if (JustinaTasks::sayAndSyncNavigateToLoc("arena", 120000)) {
						JustinaHRI::waitAfterSay("please tell me robot yes for confirm the command", 3000);
						JustinaHRI::waitAfterSay("please tell me robot no for repeat the command", 3000);
						JustinaHRI::waitAfterSay("I am ready for recieve a category two command", 3000);
