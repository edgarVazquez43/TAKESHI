...
public:
	...
	bool syncSpeech(std::string textSpeech, float timeOut, float timeSleep){
		JustinaHRI::waitAfterSay(textSpeech, int(timeOut) );
	...
	bool torsoGoTo(float goalSpine, float goalWaist, float goalShoulders, int timeOut_ms){
		return JustinaManip::torsoGoTo(goalSpine, timeOut_ms);
	}
	...
	bool findPerson(std::string person = ""){
        ...
        syncSpeech("I'am getting close to you", 3000, 2000);
	...
	bool moveActuator(float x, float y, float z, bool withLeftArm,std::string id){
	    ...
	    JustinaTasks::graspObject(x, y, z);
	...
	bool drop(){
		syncSpeech("I'am going to bring it to you", 3000, 2000);
		syncSpeech("please put your hand", 3000, 2000);
		boost::this_thread::sleep(boost::posix_time::milliseconds(10000));
		JustinaManip::raGoTo("take", 10000);
		boost::this_thread::sleep(boost::posix_time::milliseconds(10000));
		syncSpeech("I'am going handover the object", 3000, 2000);
...
void callbackCmdConfirmation(const knowledge_msgs::PlanningCmdClips::ConstPtr& msg){
	...
	if(success){
		...
		tasks.syncSpeech(ss.str(), 8000, 2000);
		...
		if(srvCltWaitConfirmation.call(srv)){
			...
			if(srv.response.success)
				tasks.syncSpeech("Ok i start to make the command", 8000, 2000);
			else
				tasks.syncSpeech("Repeate the command please", 8000, 2000);
...
void callbackCmdAnswer(const knowledge_msgs::PlanningCmdClips::ConstPtr& msg){
	...
	if(success){
		...
		if(param1.compare("a_question") == 0){
			...
			if(success){
				success = tasks.syncSpeech("I am waiting for the user question", 8000, 2000);
				knowledge_msgs::planning_cmd srv;
				srvCltAnswer.call(srv);
				if(srv.response.success)
					success = tasks.syncSpeech(srv.response.args, 8000, 2000);
				else
					success = false;
			}
		}
		else if(param1.compare("your_name") == 0 ){
			tasks.syncSpeech("Hellow my name is takeshi", 3000, 2000);
		}
		else if(param1.compare("your_team_name") == 0 || param1.compare("the_name_of_your_team") == 0 ){
			tasks.syncSpeech("Hello my team is pumas", 3000, 2000);
		}
		else if(param1.compare("introduce_yourself") == 0 ){
			tasks.syncSpeech("Hello my name is takeshi", 3000, 2000);
			tasks.syncSpeech("i am from Mexico city", 3000, 2000);
			tasks.syncSpeech("my team is pumas", 3000, 2000);
			tasks.syncSpeech("of the national autonomous university of mexico", 8000, 2000);
		}
...
int main(int argc, char **argv){
	...
	while(ros::ok()){
		...
		switch(state){
			case SM_INIT:
				if(startSignalSM){
					tasks.syncSpeech("I'm ready for the eegpsr test", 8000, 2000);
					state = SM_SAY_WAIT_FOR_DOOR;
				}
				break;
			case SM_SAY_WAIT_FOR_DOOR:
				tasks.syncSpeech("I'm waiting for the door to be open", 8000, 2000);
			...
			case SM_NAVIGATE_TO_THE_LOCATION:
				tasks.syncSpeech("I can see now that the door is open", 8000, 2000);
	            ...
	            if(!tasks.syncNavigate("arena", 120000)){
	                ...
	                if(!tasks.syncNavigate("arena", 120000)){
	                    ...
	                    if(tasks.syncNavigate("arena", 120000)){
	                    	tasks.syncSpeech("I'm ready for a spoken command", 8000, 2000);
