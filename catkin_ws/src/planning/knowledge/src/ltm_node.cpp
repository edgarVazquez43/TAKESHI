...
void callbackSaveInFile(const std_msgs::String::ConstPtr& pathName){
        std::cout << "Saving locations to file" << pathName->data<< '\n';
...
//Coyo was here
std::map<std::string, std::vector<float> > objectsLocs;
bool loadKnownObjectsLocations(std::string path,
                               std::map<std::string, std::vector<float> > &loc)
{
        //loads know objects locations
        //path to file
        //loc weird data structure to store as map key: name value float[3]
        //but implemented as vector<float> because ROS likes it that way
        std::cout << "Loading known object locations from " << path << std::endl;
        std::ifstream file(path.c_str());
        if (!file.is_open()) {
                file.close();
                return false;
        ...
        while (std::getline(file,line)) {
                ...
                std::string locName;
                if (!(iss >>locName>> loc[0] >> loc[1] >>loc[2]))
                {
                ...
                objectsLocs.insert(tmp);
        }
        std::cout << "Known Objects Locations:" << '\n';
        for (std::map<std::string,std::vector<float> >::iterator it=objectsLocs.begin(); it!=objectsLocs.end(); ++it)
        {
                std::cout << it->first << " => "
                          << it->second[0] <<" "
                          << it->second[1]<< " "
...
bool getKnownObjects(knowledge_msgs::KnownLocations::Request &req,
                     knowledge_msgs::KnownLocations::Response &res)
{
        //Server for subcribing to knwon objects
        //req is empty, res is name and a float[3] == x y theta
        std::map<std::string, std::vector<float> >::const_iterator it;
        for ( it = objectsLocs.begin(); it != objectsLocs.end(); ++it)
        {
                knowledge_msgs::MapKnownLocation map;
                map.name = it->first;
                map.value.push_back(it->second[0]);
...
bool addOrUpdateKnownObjLoc(knowledge_msgs::AddUpdateKnownLoc::Request &req, knowledge_msgs::AddUpdateKnownLoc::Response &res){
        std::vector<float> new_values = req.loc.value;
        std::map<std::string, std::vector<float> >::iterator it;
        bool updateControl = true;

        it = objectsLocs.find(req.loc.name);
        if (it != objectsLocs.end()) {
                it->second[0] = new_values[0];
                it->second[1] = new_values[1];
                if (it->second.size() > 2 && new_values.size() > 2) {
                        it->second[2] = new_values[2];
                ...
                objectsLocs[req.loc.name] = new_values;
                updateKnowLoc = false;
                initKnowLoco = true;
        }
        return true;
...
int main(int argc, char ** argv) {
        ...
        ros::ServiceServer serviceObj = nh.advertiseService(
                "/knowledge/known_objects_location",getKnownObjects);

        ros::ServiceServer serviceUpdObj =
                nh.advertiseService(
                        "/knowledge/add_update_known_object_locations", addOrUpdateKnownObjLoc);
        ...
        if (!loadKnownObjectsLocations(objectsFilePath,objectsLocs))
                std::cout << "ltm_node.-> Can not load file of known objects." << std::endl;
