...
class TakeshiRepresentation {
 ...
 public:
  ...
  static void getLocations(std::string path ,std::map<std::string,
			   std::vector<std::string> >& locations);
  static void getObjects(std::string path ,std::map<std::string,
			 std::vector<std::string> > & objects);
  static void addLocations(std::map<std::string, std::vector<std::string> >& locations,
			   std::string name, std::vector<std::string> values);
  static void addObjects(std::map<std::string, std::vector<std::string> >& objects,
			 std::string name, std::vector<std::string> values);
  static void sendAndRunCLIPS(std::string command);
  static bool speachInterpretation();
  static bool stringInterpretation(std::string strToInterpretation,
				   std::string &strInterpreted);
  static bool orderInterpeted(std::string strInterpreted, std::string &typeOrder,
			      std::string &object1, std::string &object2);
  static bool prepareInterpretedQuestionToQuery(std::string strInterpreted, std::string &query);
  static bool selectCategoryObjectByName(std::string idObject, std::string &category,
					 int timeout);
  ...
  static bool insertConfidenceAndGetCategory(std::string id, int index, float confidence,
